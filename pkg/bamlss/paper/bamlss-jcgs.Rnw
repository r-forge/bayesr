...
\if0\blind
{
  \title{\bf BAMLSS: Bayesian Additive Models for Location, Scale and Shape (and Beyond)}
...
\if1\blind
{
  ...
  \bigskip
  \begin{center}
    {\LARGE\bf BAMLSS: Bayesian Additive Models for Location, Scale and Shape (and Beyond)}
\end{center}
  \medskip
} \fi
...
\begin{abstract}
  ...
  exploits distributions, estimation techniques (posterior mode or posterior mean), and model terms
  ...
  climatology, as well as a Cox model for continuous time with space-time interactions.
\end{abstract}

\noindent%
{\it Keywords:} GAMLSS, distributional regression, MCMC, \proglang{BUGS}, \proglang{R}, software
\vfill
...
a response variable to allow for modeling the mean (location) as well
...
of the distribution is linked to an additive predictor in similar fashion as for the
well-established generalized additive model (GAM,~\citealp{bamlss:Hastie+Tibshirani:1990}).
...
\citep{bamlss:Ruppert+Wand+Carrol:2003}. In a fully Bayesian setting this
...
\citep{bamlss:Fahrmeir+Kneib+Lang:2004, bamlss:Fahrmeir+Kneib+Lang+Marx:2013}.
...
asymptotic properties fail. This is specifically the case in more complex GAMLSS models
\citep{bamlss:Klein+Kneib+Lang:2015}. In addition, extensions such as variable selection,
...
inferential problems has been receiving increasing attention. Existing estimation engines already
provide infrastructures for a number of regression problems exceeding univariate responses, e.g.,
for multinomial, multivariate normal, censored, or truncated response variables, etc.

However, many engines use different model setups and output 
formats, making it difficult to compare properties of
...
\pkg{WinBUGS}~\citep{bamlss:Lunn+Thomas+Best+Spiegelhalter:2000}; etc.
...
new models and algorithms. Second, to compare existing algorithms and samplers.
Third, to easily integrate existing implementations. A proof of concept is given in the
corresponding \proglang{R} package \pkg{bamlss} \citep{bamlss:Umlauf+Klein+Zeileis:2016}.
...
used to estimate numerous (complex) models along with the corresponding
``Lego bricks'' in Section~\ref{sec:legobricks}.
Section~\ref{sec:strategies} describes computational strategies for implementation before
Section~\ref{sec:illustrations} illustrates the concept using two
...
\end{equation}
based on $j = 1, \ldots, J_k$ unspecified (possibly nonlinear) functions $f_{jk}(\cdot)$, applied to each row
of the generic data matrix $\mathbf{X}$, encompassing all available covariate information.
The corresponding parameters $\boldsymbol{\beta}_k = (\boldsymbol{\beta}_{1k}, \ldots, \boldsymbol{\beta}_{J_kk})^\top$ are
typically regression coefficients pertaining to model matrices
$\mathbf{X}_k = (\mathbf{X}_{1k}, \ldots, \mathbf{X}_{J_kk})^\top$,
whose structure only depend on the type of covariate(s) and prior assumptions about $f_{jk}( \cdot )$.
For notational convenience, the vector of function evaluations (across all observations
$i = 1, \ldots, n$) is also denoted by
\begin{equation} \label{eqn:functions}
\mathbf{f}_{jk} = f_{jk}(\mathbf{X}_{jk}; \boldsymbol{\beta}_{jk})
  = (f_{jk}(\mathbf{x}_{1}; \boldsymbol{\beta}_{jk}), \ldots,
  f_{jk}(\mathbf{x}_{n}; \boldsymbol{\beta}_{jk}))^{\top},
\end{equation}
where $\mathbf{X}_{jk}$ ($n \times m_{jk}$) is the design matrix of the $j$-th term for
the $k$-th parameter. In the case where it is derived through a basis function approach, it
can be written as $\mathbf{f}_{jk} = \mathbf{X}_{jk}\boldsymbol{\beta}_{jk}$.
...
(STAR,~\citealp{bamlss:Fahrmeir+Kneib+Lang:2004}), in this paper
we relax this assumption and let $f_{jk}(\cdot)$ be an unspecified composition of covariate data
and regression coefficients. A simple
...
The matrix $\mathbf{U}_{jk}$ is derived from a spectral decomposition of the penalty matrix
...
\citealp{bamlss:Ruppert+Wand+Carrol:2003, bamlss:Wood:2004, bamlss:Fahrmeir+Kneib+Lang+Marx:2013}).
...
\ell(\boldsymbol{\beta} ; \mathbf{y}, \mathbf{X}) =
  \sum_{i = 1}^n \log \, d_y(y_i ; \theta_{i1} = h_1^{-1}(\eta_{1}(\mathbf{x}_i; \boldsymbol{\beta}_1)), \ldots,
  \theta_{iK} = h_K^{-1}(\eta_{K}(\mathbf{x}_i; \boldsymbol{\beta}_K)))
\end{equation*}
...
Assigning prior distributions $p_{jk}( \cdot )$ to the individual components yields
the log-posterior
...
\item \label{E2} Solving high-dimensional integrals, e.g., for posterior mean or median
  estimation.
\end{enumerate}
For the possibly complex models within the BAMLSS framework, \ref{E1} and
...
using updating function $U_{jk}( \cdot )$ for an individual model term
...
Section~\ref{sec:coxreg}). In problem \ref{E2} using MCMC it is common to
...
as outlined in Algorithm~\ref{fig:algodesign}. The algorithm initializes all
...
multi-dimensional $f_{jk}( \cdot )$ with multiple smoothing variances.
Therefore, we propose to estimate $\boldsymbol{\tau}_{jk}$
using a goodness-of-fit criterion within the stepwise selection from
Algorithm~\ref{fig:umode}, similar to \citet{bamlss:Belitz+Lang:2008}.
In each updating step in \ref{fig:algodesign} each
...
one-dimensional search that is relatively fast and straightforward. The algorithm
does not guarantee a global minimum, % given the goodness-of-fit criterion,
however, the solution is at least ``close'' and serves as the starting point for full MCMC. Optimization speed can be
further increased if for a given search interval only a grid of possible values for each
$\tau_{ljk}$ is used.

The MCMC updating usually either accepts or rejects samples of the parameters and smoothing
...
estimating GAM-type models (building block \ref{item:3}). In addition,
Table~1 of the supplemental material provides a more detailed overview of model terms and prior
structures.
...
not of full rank and therefore $d_{\boldsymbol{\beta}_{jk}}( \cdot )$ is partially improper.
...
However, the inverse gamma prior is very sensitive to the choice of $\mathbf{a}_{jk}$ and $\mathbf{b}_{jk}$
if $\tau_{ljk}$ is close to zero. Therefore,
\citet{bamlss:Gelman:2006} proposes the half-Cauchy prior which has the desirable property
that for $\tau_{ljk} = 0$ the density is a nonzero constant, whereas the density of the inverse
gamma for $\tau_{ljk} \rightarrow 0$ vanishes (see also \citealp{bamlss:Polson+Scott:2012}).
Another question is the actual choice of hyper-parameters. A recent suggestion reducing
...
different resolutions. Whenever there is such a nested structure
...
The construction of suitable updating functions $U_{jk}( \cdot )$ for solving \ref{E1}
and \ref{E2} can be carried out in many ways. (Note again that the general algorithm
\ref{fig:algodesign} does not restrict to a specific iterative procedure.)
In the following we describe commonly-used quantities that can be used for estimation of BAMLSS.
Moreover, this section highlights the ``Lego'' properties obtained from
the gradient-based updating in \ref{E1} and \ref{E2}.
...
minimizing information criteria such as the Akaike (AIC) or Bayesian information criterion (BIC).
See also Algorithm~\ref{fig:umode} for an adaptive stepwise approach
...
\end{equation}
with the following score vector $\mathbf{s}(\boldsymbol{\beta})$ and Hessian matrix $\mathbf{H}_{ks}(\boldsymbol{\beta})$
($k, s = 1, \dots, K$)
\begin{eqnarray*} \label{eqn:score}
...
\end{eqnarray*}
By chain rule, the part of the score vector involving
...
$\boldsymbol{\eta}_k$ and $\boldsymbol{\theta}_k$ (building block \ref{item:6a}), the inverse
link functions (\ref{item:4}), the predictor $\boldsymbol{\eta}_k$
with respect to coefficients $\boldsymbol{\beta}_k$ (\ref{item:5}). Similarly, the components of
$\mathbf{H}_{ks}$ including $\ell(\boldsymbol{\beta}; \mathbf{y}, \mathbf{X})$ (\ref{item:7a}) can be written as
...
Using a $k$-partitioned updating scheme as in (\ref{eqn:blockupdate}) updating functions
...
(in the supplemental material Section~3 the detailed derivations are presented), i.e., the algorithm
...
$\mathbf{z}_k - \boldsymbol{\eta}_{k, -j}^{(t+1)}$ can be used. Note also that this
is equivalent to updating function
\begin{equation} \label{eqn:blockblocknewton}
...
involving building blocks \ref{item:6a}, \ref{item:7a} and \ref{item:8} (see also the
supplemental material Section~3 the detailed derivations).
...
E(\boldsymbol{\beta}, \boldsymbol{\tau} | \, \mathbf{y}, \mathbf{X}, \boldsymbol{\alpha}) =
  \int\int (\boldsymbol{\beta}, \boldsymbol{\tau})^\top
  \pi(\boldsymbol{\beta}, \boldsymbol{\tau} ; \mathbf{y}, \mathbf{X}, \boldsymbol{\alpha})
  d(\boldsymbol{\beta}, \boldsymbol{\tau}).
\end{equation*}
...
$\tilde{a}_{jk} = \frac{1}{2}rk(\mathbf{K}_{jk}) + a_{jk}$,
...
As noted in Section~\ref{sec:bricksmodelfit}, depending on the structure of the priors
(\ref{eqn:gprior}) parameters $\boldsymbol{\tau}_{jk}$ cannot be estimated by maximization of the
log-posterior (\ref{eqn:logpost}), e.g., this is the case for GAM-type models using normal priors (\ref{eqn:mvnormprior}) where
$\boldsymbol{\tau}_{jk}$ represents smoothing variances.

Then, goodness-of-fit criteria like the Akaike information criterion (AIC), or the corrected AIC,
as well as the Bayesian information criterion (BIC), amongst others, are commonly used for selecting
the smoothing variances. Estimation of model complexity is based on the so-called
equivalent degrees of freedom (EDF), calculated by
$
...
even for more complex likelihood structures.
...
From a computational perspective it can be difficult to derive the full Hessian
...
\begin{enumerate}
  ...
  \item A generic method for setting up model terms and a \code{model.frame} along with the
    ...
    e.g., using mixed model representation for smooth terms
...
\put(0, 5){\fcolorbox{black}{heat5}{\framebox(2, 0.7)[c]{\scriptsize \texttt{formula}}}}
\put(2.5, 5){\fcolorbox{black}{heat5}{\framebox(2, 0.7)[c]{\scriptsize \texttt{family}}}}
\put(5, 5){\fcolorbox{black}{heat5}{\framebox(2, 0.7)[c]{\scriptsize \texttt{data}}}}
\put(2.5, 3.5){\fcolorbox{black}{heat4}{\framebox(2, 0.7)[c]{\scriptsize \texttt{model.frame}}}}
\put(2.5, 2.5){\fcolorbox{black}{heat3}{\framebox(2, 0.7)[c]{\scriptsize \texttt{transformer}}}}
\put(6, 3.5){\fcolorbox{black}{heat2}{\framebox(2, 0.7)[c]{\scriptsize \texttt{optimizer}}}}
...
\put(6, 2.5){\fcolorbox{black}{heat1}{\framebox(2, 0.7)[c]{\scriptsize \texttt{sampler}}}}
\put(6, 1.5){\fcolorbox{black}{heat2}{\framebox(2, 0.7)[c]{\scriptsize \texttt{results}}}}
\put(1, 0){\fcolorbox{black}{heat5}{\framebox(2, 0.7)[c]{\scriptsize \texttt{summary}}}}
\put(3.5, 0){\fcolorbox{black}{heat5}{\framebox(2, 0.7)[c]{\scriptsize \texttt{plot}}}}
\put(6, 0){\fcolorbox{black}{heat5}{\framebox(2, 0.7)[c]{\scriptsize \texttt{select}}}}
\put(8.5, 0){\fcolorbox{black}{heat5}{\framebox(2, 0.7)[c]{\scriptsize \texttt{predict}}}}
...
\end{picture}
\caption{\label{fig:blocks} Flexible functional BAMLSS architecture. Each building block in the
  middle part can be exchanged by the user. Usually only the \code{optimizer} and \code{sampler}
  functions are adapted for implementing new models.}
\end{figure}
...
-- based on recommended \proglang{R} package \pkg{mgcv} --
...
linear regression model with a response variable \code{y} and two regressors:
\code{y} $\sim$ \code{x1 + x2}.
But it can also encompas smooth terms in further covariates:
\code{y} $\sim$ \code{x1 + x2 + s(x3) + s(x4, x5)}.
Or it may even have different additive predictors for different model parameters:
\code{list(}$\,$\code{y} $\sim$ \code{x1 + x2 + s(x3) + s(x4),}
\code{sigma} $\sim$ \code{x1 + x2 + s(x3)}$\,$\code{)}, e.g.,
...
objects in order to plug in different Lego bricks for \ref{item:1}--\ref{item:8}. Family objects from the \pkg{gamlss} suite are readily supported.
...
To give a brief insight how to use \pkg{bamlss} we illustrate the basic steps on a small textbook
...
@
where \fct{s} is the smooth term constructor from \pkg{mgcv}. The model is
then fitted by
<<eval=FALSE>>=
data("mcycle", package = "MASS")
b <- bamlss(f, data = mcycle, family = "gaussian")
@
<<cache-toymodel, echo=FALSE, eval=TRUE, results=hide>>=
data("mcycle", package = "MASS")
if(!file.exists("toymodel.rda")) {
set.seed(123)
b <- bamlss(f, data = mcycle, family = "gaussian",
  n.iter = 12000, burnin = 2000, thin = 10)
b0 <- bamlss(accel ~ s(times,k=20), data = mcycle, family = "gaussian",
  n.iter = 12000, burnin = 2000, thin = 10)
save(b0, b, file = "toymodel.rda")
} else {
load("toymodel.rda")
}
@
The returned object is of class \code{"bamlss"} for which generic extractor functions like
\fct{summary}, \fct{plot}, \fct{predict}, etc., are provided. E.g., the posterior mean function
based on MCMC samples for parameter $\mu$ can be computed by
<<eval=FALSE>>=
p <- predict(b, model = "mu", FUN = mean)
@
\begin{figure}[!t]
\setkeys{Gin}{width=0.27\textwidth}
\centering
<<echo=FALSE, fig=TRUE, width=3, height=3.4>>=
par(mar = c(4.1, 4.1, 2.1, 1.1))
p <- predict(b0, model = "mu", FUN = c95)
...
mtext(expression(paste(accel, "~ N(", mu, "=", f(times), ",", log(sigma), "=", beta[0], ")")),
  side = 3, line = 1)
@
\hspace*{4ex}
\setkeys{Gin}{width=0.54\textwidth}
<<echo=FALSE, fig=TRUE, width=6, height=3.4>>=
par(mar = c(4.1, 4.1, 0, 1.1), mfrow = c(1, 2), oma = c(0, 0, 2.1, 0))
...
@
\caption{\label{fig:toymodel} Estimated effects of the Gaussian location-scale model using the
  simulated motorcycle accident data together with $95\%$ credible bands computed from the empirical 
  quantiles of the MCMC samples (gray shaded areas). The left panel shows the estimated function
  for parameter $\mu$ using a constant scale parameter $\log(\sigma) = \beta_0$. The middle panel
  shows the estimated function for parameter $\mu$ using $\log(\sigma) = f(\texttt{times})$.
  The right panel shows the corresponding estimated effect on $\log(\sigma)$.}
\end{figure}
where argument \code{FUN} can be any function, e.g., a function computing credible intervals from
the empirical quantiles of the MCMC samples. In Figure~\ref{fig:toymodel} the estimated function
for a Gaussian model with constant scale $\log(\sigma) = \beta_0$ is shown in the left panel.
Although the mean
function seems to modeled appropriately, the plot shows that the $95\%$ credible bands are too wide
in the beginning and the end of the experiment. The middle panel shows the estimated function
of the heteroskedastic model from above. Here, the credible bands follow much better the
distribution of the data driven by the estimated variance function shown in the right panel. Uncertainty is low in the beginning and goes up until about $35$ milliseconds and slightly
decreases afterwards. 

\section{Illustrations} \label{sec:illustrations}

\subsection{Censored heteroscedastic precepitation climatology} \label{sec:censreg}
...
$$
y^\star \sim \mathcal{N}(\mu, \sigma^2),\,\, \mu = \eta_{\mu},\,\,
  \log(\sigma) = \eta_{\sigma},\,\, y = \max(0, y^\star).
$$
Because precipitation in the Alps is driven by the season and local characteristics, e.g.,
differing altitude form north to south, we use the following predictor for $\mu$ and $\sigma$:
$$
...
implemented (the detailed expressions are provided in the supplemental material Section~2):
\begin{enumerate}[leftmargin=*,align=left]
\item[\ref{item:1}.] The density function of a Gaussian distribution left censored at zero.
\item[\ref{item:6b}.] Score vectors $\mathbf{u}_k = \partial \ell(\boldsymbol{\beta}; \mathbf{y}, 
  \mathbf{X}) / \partial \boldsymbol{\eta}_k$.
...
Intel i7-2600 3.40GHz processors running the model takes approximately 17 hours.
For each core, the first $2000$ samples are withdrawn and only every 10th sample is saved.
...
\begin{figure}[t!]
\centering
\includegraphics[width=0.36\textwidth]{figures/rainmodel-data-stations}\includegraphics[width=0.36\textwidth]{figures/rainmodel-data-hist}
\caption{\label{fig:rainmodeldata} Distribution of available meteorological stations, left panel,
  and daily square root transformed precipitation values, right panel.}
\end{figure}

\begin{figure}[p!]
\centering
\includegraphics[width=0.36\textwidth]{figures/rainmodel-effects-season-mu}\includegraphics[width=0.36\textwidth]{figures/rainmodel-effects-season-sigma} \\[0.5cm]
\includegraphics[width=0.36\textwidth]{figures/rainmodel-effects-spatial-mu}\includegraphics[width=0.36\textwidth]{figures/rainmodel-effects-spatial-sigma} \\[0.5cm]
\includegraphics[width=0.72\textwidth]{figures/rainmodel-effects-predict}
\caption{\label{fig:rainmodeleffects} Estimated effects of the precipitation model showing the spatial
  variation of the seasonal effects together with the spatial main effects, 1st and 2nd row,
...
The distribution of the $5838$ dwelling fires in 2015 together with the
fire stations is shown in Figure~\ref{fig:londonfiredata} (a precompiled version of the data is
available in the \pkg{bamlss} package). The top left panel indicates that both,
fire stations and fire events, are spread all over London with a higher density in the city center
which is brought out more clearly by the heatmap in the bottom left panel. The panels on the
right-hand side pertain to the arrival time and show that overall about 30\% of these were
greater than six minutes (bottom right) with most of these occurring at the borders of London
(top right).

\begin{figure}[t!]
\centering
\includegraphics[width=0.72\textwidth]{figures/firemodel-data}
\caption{\label{fig:londonfiredata} Distribution of dwelling fires, fire stations, and arrival
  times in London, 2015. The upper right panel shows that the percentage rate of arrival times
  $> 6$ minutes is higher close to the borders of London, besides, most fires occur in the city
  center illustrated in the lower left panel.}
\end{figure}
...
Moreover, we also relax the time-varying predictor to
...
which is of prime interest in this analysis. For full Bayesian inference
...
(the detailed expressions are provided in the supplemental material Section~3):
\begin{enumerate}[leftmargin=*,align=left]
\item[\ref{item:1}.] The log-likelihood function of the continuous time Cox model.
\item[\ref{item:6a}.] For derivative-based estimation using Algorithm~\ref{fig:umode} and for
  MCMC simulation with Algorithm~\ref{fig:mcmc}, the score vectors and Hessian need to be computed.
\item[\ref{item:7a}.] The elements of the Hessian w.r.t.\ $\boldsymbol{\beta}_{\lambda}$. Note that
  these cannot be computed by fragmenting with the chain rule to obtain building block
  \ref{item:7b} and IWLS updating functions, see the supplemental material Section~3 for details.
\item[\ref{item:6b} \& \ref{item:7b}.] However, constructing updating functions for the
  time-constant part
  $\eta_{\gamma}$ again yields an IWLS updating scheme based on building block \ref{item:7b}.
\end{enumerate}

\begin{figure}[p!]
\centering
\includegraphics[width=0.36\textwidth]{figures/firemodel-effects-baseline}\includegraphics[width=0.36\textwidth]{figures/firemodel-effects-prob} \\[0.5cm]
\includegraphics[width=0.36\textwidth]{figures/firemodel-effects-fsintens}\includegraphics[width=0.36\textwidth]{figures/firemodel-effects-spatial-td} \\[0.5cm]
\includegraphics[width=0.36\textwidth]{figures/firemodel-effects-daytime-curves}\includegraphics[width=0.36\textwidth]{figures/firemodel-effects-spatial-tc}
...
the sampler function \fct{cox.mcmc} for MCMC simulation (which are part of the corresponding
\pkg{bamlss} family object \fct{cox\_bamlss}).
%
...
\begin{figure}[t!]
\centering
\includegraphics[width=0.27\textwidth]{figures/firemodel-daytime-t1}\includegraphics[width=0.27\textwidth]{figures/firemodel-daytime-t2}\includegraphics[width=0.27\textwidth]{figures/firemodel-daytime-t3} \\[0.5cm]
\includegraphics[width=0.27\textwidth]{figures/firemodel-daytime-t4}\includegraphics[width=0.27\textwidth]{figures/firemodel-daytime-t5}\includegraphics[width=0.27\textwidth]{figures/firemodel-daytime-t6} \\[0.5cm]
\includegraphics[width=0.27\textwidth]{figures/firemodel-daytime-t7}\includegraphics[width=0.27\textwidth]{figures/firemodel-daytime-t8}\includegraphics[width=0.27\textwidth]{figures/firemodel-daytime-t9}
\caption{\label{fig:londonfiredaytime} Estimated spatial varying time-of-the-day effect. The figure shows
  that in the early morning hours, as well as in the afternoon, the effect on the probability that
  an engine arrives are negative in certain areas, however, relatively small compared to the other
  estimated effects.}
\end{figure}
...
One drawback of our framework when using a MCMC engine with highly complex predictor structures in
the distribution parameters and big data sets is a possibly long computing time. To overcome this
problem one may either resort to parallel computing facilities -- if available -- or approximative,
faster methods like integrated nested Laplace approximations \citep{bamlss:Rue+Martino+Chopin:2009}.
The latter however, is currently restricted to distributions with only one predictor and hence not
usable for instance a heteroscedastic Gaussian model.

Interesting extensions in the future include efficient matrix transformations for additional effect
types based on Gaussian processes \citep{bamlss:Paciorek:2007} and the broadening of the available model classes in bamlss such as state-space models \citep{bamlss:Carter+Kohn:1994}.
...
\item[Algorithmic details:] Detailed information about algorithmic derivations and ``Lego bricks''
  used in the main manuscript are provided in the accompanying online supplementary material
  \code{.pdf}.

\item[\proglang{R}-package:] \proglang{R}-package \pkg{bamlss}, including the data sets for
  reproducing the illustrations, is available at \url{http://CRAN.R-project.org/package=bamlss}.

\item[Code:] The \proglang{R} code for reproducing the models is provided in the script
  \code{models.R}, for reproducing all figures in the script \code{figures.R}.
