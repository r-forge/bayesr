...
(3)~Enhanced inference infrastructure, typically Bayesian, beyond classical frequentist significance tests.
...
In terms of software infrastructure, the \proglang{R} ecosystem already provides a
rich variety of packages that combine several -- but not all -- of the aspects discussed above.
%
\begin{itemize}

\item \emph{GAMs and GAMLSSs}
are available in a number of packages, most notably the \pkg{mgcv} package \citep{bamlss:Wood:2017}
and also the \pkg{gamlss} family of packages \citep{bamlss:Stasinopoulos:Rigby:2007}
and \pkg{VGAM} \citep{bamlss:Yee:2009}. The latter two are notable for their support
of a wide range of response distributions. However, for complex predictor structures
and response distributions beyond the exponential family, estimation may be challenging
or subject to numerical instabilities. In contrast, \pkg{mgcv} excels at providing
highly-optimized algorithms for general smooth models \citep{bamlss:Wood+Pya+Saefken:2016}
as well as the dedicated \fct{bam} function for big data that is long and/or wide
\citep{bamlss:Wood+Li+Shaddick:2017}.
...
\pkg{gamboostLSS} \citep{bamlss:Hofner+Mayr+Schmid:2014}. However, obtaining MCMC samples from
the posterior distributions corresponding to such models is not easily available in these
packages.
...
binomial logit model using the default MCMC algorithm can be fitted. First, the model formula
specified with
...
parameters suggest close to i.i.d.\ samples from the posterior distribution. As mentioned above, the
...
package \pkg{mgcv} \citep{bamlss:Wood:2019} infrastructures. To estimate a spline model instead of
a polynomial model for variable \code{age} the model formula only needs to be slightly adapted
<<>>=
f <- participation ~ income + education +
  youngkids + oldkids + foreign + s(age, k = 10)
@
...
\subsection{Location-scale model} \label{sec:locscalemodel}
...
impact (variable \code{times}). To estimate a location-scale model with
$$
...
weighted least squares updating step  
...
\multirow{4}{*}{Estimation} & \multirow{2}{*}{Optimizer} & \code{bfit()}, \code{bbfit()},
 ...
 & &\code{cox\_mode()}, \code{jm\_mode()}  \\ \cline{2-3}
 & \multirow{2}{*}{Sampler} & \code{GMCMC()}, \code{BayesX()}, \code{JAGS()} \\
 & & \code{cox\_mcmc()}, \code{jm\_mcmc()} \\ \hline
