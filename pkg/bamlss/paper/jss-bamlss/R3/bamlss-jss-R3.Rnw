...
%% references with long author lists to always be abbreviated
\shortcites{bamlss:INLA,bamlss:Waldmann+Taylor+Klein+Kneib+Pressler+Schmid+Mayr:2016,bamlss:stan,zeileis2019colorspace,bamlss:Leon:2020,fouillet2008,westra2014}
...
\Keywords{Backfitting, distributional regression, gradient boosting, MCMC, penalization, probabilistic forecasting, \proglang{R}}
\Plainkeywords{Backfitting, distributional regression, gradient boosting, MCMC, penalization, probabilistic forecasting, R}

\Abstract{
  Over the last decades, the challenges in applied regression and in predictive modeling have
  been changing considerably: (1)~More flexible regression model specifications are needed 
  as data sizes and available information are steadily increasing, consequently
  demanding for more powerful computing infrastructure.
  (2)~Full probabilistic models by means of distributional regression
  -- rather than predicting only some underlying individual quantities from the distributions such as means or
  expectations -- is crucial in many applications.
  (3)~Availability of Bayesian inference has gained in importance both as an appealing framework for
  regularizing or penalizing complex models and estimation therein as well as a natural
  alternative to classical frequentist inference. However, while there has been a lot of
  research on all  three challenges and the development of corresponding software packages,
  a modular software implementation that allows to easily combine all three aspects has not yet been 
  available for the general framework of distributional regression.
  
  To fill this gap,
  the \proglang{R} package \pkg{bamlss} is introduced for Bayesian additive models for location,
  scale, and shape (and beyond) -- with the name reflecting the most important distributional
  quantities (among others) that can be modeled with the software.
  At the core of the package are algorithms for highly-efficient Bayesian
  estimation and inference that can be applied to generalized additive models or
  generalized additive models for location, scale, and shape, or more general
  distributional regression models. However, its building
  blocks are designed as ``Lego bricks'' encompassing various distributions (exponential family,
  Cox, joint models, etc.), regression terms (linear, splines, random effects, tensor
  products, spatial fields, etc.), and estimators (MCMC, backfitting, gradient boosting, lasso, etc.).
  It is demonstrated how these can be easily combined to make classical models more
  flexible or to create new custom models for specific modeling challenges.
}
...
\Address{
  ...
  URL: \url{https://eeecon.uibk.ac.at/~umlauf/},\\
  \phantom{URL: }\url{https://www.zeileis.org/}\\
...
\SweaveOpts{engine = R, eps = FALSE, keep.source = TRUE, prefix.string=Figures/jss2924}
...
(1)~Large data sets that can be both long (many observations) and/or wide (many variables or
complex effect types).
(2)~Probabilistic forecasts that capture the entire distribution and not
only its mean or expectation.
(3)~Enhanced inference infrastructure, typically Bayesian, broadening classical frequentist 
methodology. A popular framework to combine flexible regression with probabilistic modeling
are generalized additive models \citep[GAMs,][]{bamlss:Hastie+Tibshirani:1990}, later extended
to generalized additive models for location, scale, and shape
\citep[GAMLSS,][]{bamlss:Rigby+Stasinopoulos:2005},
also known as Bayesian structured additive distributional regression 
\citep{bamlss:Klein+Kneib+Lang+Sohn:2015} which
encompasses (generalized) linear models \citep[GLMs,][]{bamlss:Nelder+Wedderburn:1972} as special cases.
Bayesian inference in these models can be seen as a natural framework
...
techniques using standard algorithms like iteratively weighted least squares
(IWLS, \citealp{bamlss:Gamerman:1997}) or
Markov chain Monte Carlo (MCMC) might not be feasible. Instead, regularized estimation
techniques such as lasso or boosting
\citep{bamlss:Friedmann+Hastie+Tibshirani:2010,bamlss:Mayr+Fenske+Hofner:2012}
might be necessary or further advanced custom algorithms \citep{bamlss:Wood:2017}. Hence,
to facilitate addressing all challenges and needs simultaneously -- independent of a specific 
estimation strategy and/or fitting algorithm -- the \pkg{bamlss} package for the \proglang{R}
...
not only the response distribution is a ``Lego brick'' (as in a classical GLM) or the
...
of well-known packages for regression models in the \proglang{R} ecosystem, whose
implementations are designed to be extremely flexible.
%
\begin{itemize}
...
supports (user-defined) smooth additive terms of general type (e.g., neural networks and
regression trees), however, inference is mainly supported only
...
that is long and/or wide \citep{bamlss:Wood+Li+Shaddick:2017}. All these packages rely on 
frequentist estimation strategies. Moreover, the package provides
sophisticated infrastructure for generating new classes of smooth terms (which is fully
...
\pkg{JAGS}, \pkg{Stan}, or the \proglang{BUGS} family of packages \citep{bamlss:Lunn+Thomas+Best+Spiegelhalter:2000,bamlss:Goudie+Turner+Angelis:2020}.
However, for complex models -- e.g., using large data sets, spatial effects, or
higher-order interactions -- sampling times from these generic MCMC engines can become long, 
sometimes prohibitively long. This has been addressed by dedicated packages for Bayesian
additive models, e.g., with the standalone package
\pkg{BayesX}~\citep{bamlss:Brezger+Kneib+Lang:2005, bamlss:Belitz+Brezger+Kneib+Lang+Umlauf:2017} 
being the first to provide highly-efficient
sampling schemes for very large data sets as well as spatial/multilevel models and structured 
additive distributional regression. An \proglang{R} interface is available in \pkg{R2BayesX} 
\citep{bamlss:Umlauf:2015}. Instead of fully Bayesian MCMC
...
\item \emph{Regularized estimation} and explicit variable selection might be necessary, though,
for going beyond the models described above, especially for large/wide data with many potential 
regressors and corresponding effects/interactions/etc. Widely-used approaches for this include
the lasso, e.g., as available for GLM-type models in the \proglang{R} package \pkg{glmnet} 
\citep{bamlss:Friedmann+Hastie+Tibshirani:2010}, or gradient boosting as available for
GAMLSS-type models in the \proglang{R} package \pkg{gamboostLSS}
\citep{bamlss:Hofner+Mayr+Schmid:2014}. However, many packages do not cover the Bayesian
posterior estimation parts.
...
of the provided infrastructure. Section~\ref{sec:flexreg}
...
cases once more the flexibility of the provided modeling infrastructure.
...
computational advantages when using very large data sets
...
@
Then, to reproduce the results the seed of the random number generator is set and
...
@
Note that the default number of iterations (\code{n.iter}) for the MCMC sampler is 1200, the
burnin-phase \code{burnin} is 200 and thinning (\code{thin}) is 1. The reason is that during the
modeling process, users usually want to obtain first
...
Note that the model contains a quadratic term for variable \code{age} in order to capture nonlinearities.
The resulting object \code{b} is of class \code{"bamlss"} for which standard
...
coefficients, which are calculated by the upstream optimizer algorithm (note that the column
is named parameters, because optimizer functions can in principle return any type of parameters).
Besides, more results from the optimizer are reported at the very end of the output:
the corrected AIC (\code{AICc}, \citealp{bamlss:aic1, bamlss:aic2}), the equivalent degrees of freedom
(\code{edf}), the log-likelihood (\code{logLik}), etc. In addition, there are also extractor 
functions in \pkg{bamlss} for information criteria like the DIC (function \fct{DIC}) and the widely 
applicable information criterion (WAIC, \citealp{watanabe2010asymptotic}, function \fct{WAIC}), or 
the out-of-sample continuous rank probability score
(CRPS, \citealp{bamlss:Gneiting+Raftery:2007, bamlss:Gneiting+Balabdaoui+Raftery:2007}, function
\fct{CRPS}).
Note that \fct{CRPS} approximates numerically, while the \pkg{scoringRules} package 
\citep{bamlss:Jordan+Krueger+Lerch:2018} can compute the CRPS 
very efficiently for some distributions. The usage of the provided functions is similar to the
generic \fct{AIC} and \fct{BIC}, e.g., the DIC can be computed with
<<>>=
DIC(b)
@
Before proceeding the analysis, users usually perform additional convergence checks of the
MCMC chains by looking at traceplots and autocorrelation (besides acceptance probabilities).
<<eval=FALSE>>=
...
\caption{\label{fig:logit_traceplots} Logit model, MCMC trace (left panel),
  autocorrelation for the intercept (middle panel), maximum autocorrelation for all
  parameters (right panel).}
\end{figure}
These are visualized in Figure~\ref{fig:logit_traceplots} and reveal approximate convergence of
the MCMC chains, i.e., there is no visible trend, and the very low autocorrelation shown for
the intercept and the maximum autocorrelation calculated as the maximum for each lag across all 
...
parameter (arguments \code{n.iter}, \code{burnin} and \code{thin}), to make the significant bar at lag one disappear.
Note that the function call would compute all trace and autocorrelation plots, however,
...
which includes a rich infrastructure for further convergence diagnostic checks,
...
Besides the supported infrastructure from the \pkg{mgcv} package, it is also possible to
...
@
The optimum shrinkage parameter $\lambda$ is selected by the AIC (another option is
\code{criterion = "BIC"}). Arguments \code{upper} and
...
@
\caption{\label{fig:lasso} Left panel, AIC curve with optimum shrinkage parameter $\lambda$
...
The AIC curve and the coefficient paths including the optimum shrinkage parameter $\lambda$ can
be visualized with
<<eval=FALSE>>=
pathplot(b)
@
Figure~\ref{fig:lasso} shows the AIC curve and coefficient paths for \code{cage}. The AIC curve
assumes a minimum at the vertical gray dashed line. The coefficient paths obviously depict that
the algorithm can either shrink categories out of the model (shrink to zero), or even fuses them.
In the right panel of Figure~\ref{fig:lasso}, the estimated effect of the categorized variable
\code{age} is shown. The effect is computed by predicting without
intercept using the optimum stopping iteration, which is selected by AIC and can be extracted with
...
@
Using the fused lasso estimation nonlinearities can be identified again, similar to the
spline based estimate in the right panel of Figure~\ref{fig:logit_effects}.

\subsection{Location-scale model} \label{sec:distmodel}

Here, we extend the framework and estimate a distributional regression model that not
only captures the mean (or location) of the response variable but also its variance
(or scale). As an example, we employ the number of weekly fatalities in Austria from
2000--2020 (up to week~46 in 2020) as obtained from the Eurostat data base
(\url{https://ec.europa.eu/eurostat/}). The data is available in the \pkg{bamlss} package
as \code{fatalities}, providing the number (\code{num}) of fatalities in each
\code{year} and \code{week}. It can be loaded with
<<>>=
data("fatalities", package = "bamlss")
@
The idea of the subsequent analysis is to estimate a reference mortality model based
on the data from 2000--2019 prior to the COVID-19 (Corona virus disease 2019) crisis
in order to bring out graphically the excess mortality in 2020. Excess mortalities are
often employed for assessing the effects of exceptional events such as
pandemics \citep{bamlss:Leon:2020} or natural catastrophes \citep{fouillet2008}.
First, we split the data into the corresponding subsets.
<<eval=TRUE,echo=TRUE>>=
d19 <- subset(fatalities, year <= 2019)
d20 <- subset(fatalities, year >= 2020)
@
To capture the long-term seasonal trend of the fatality number distribution,
we employ a simple model here: log-fatalities are assumed to be
normally distributed with smooth seasonal variations in both mean and variance.
As shown below the log-transformation stabilizes skewness and variance in the data
somewhat so that a normal model works sufficiently well. Cyclic splines with respect
to the week of the year are employed to capture the smooth seasonal trends while assuring
that the values at the beginning and the end of the year match. The model formula
is now a list with elements for the mean of \code{log(num)} (corresponding to parameter
\code{mu}) and standard deviation \code{sigma} of the normal distribution.
<<>>=
f <- list(
...
regression spline. (Other smooth terms such as \fct{te} or \fct{ti} could be included
in the same way.) Based on this \fct{bamlss} is used to estimate a full Bayesian
regression model using the \fct{NO} normal family from the \pkg{gamlss.dist} package.
<<echo=FALSE, results=hide>>=
...
b <- bamlss(f, data = d19, family = NO)
@
The resulting estimated effects along with their 95\% credible intervals can be easily
visualized using the \fct{plot} method:
<<eval=FALSE>>=
...
@
\caption{\label{fig:fatmodel_effects} Estimated effects of \code{week} on the mean \code{mu}
  and standard deviation \code{sigma} of the normal model. The gray shaded areas represent 95\% credible intervals.}
\end{figure}
The resulting displays are shown in Figure~\ref{fig:fatmodel_effects} depicting a clear nonlinear
relationship for both distribution parameters. The left panel shows that mean log-fatalities are much higher in winter
than in summer with a peak around February matching the highest risk for influenza and other viral
infections in Austria. The right panel shows that the standard deviation is also highest at around
the same time but that there is another local maximum in the summer months, possibly related to 
recurrent heat waves that can be quite stressful for the cardiovascular system \citep{fouillet2008}.

\begin{figure}[t!]
\centering
\setkeys{Gin}{width=0.75\textwidth}
<<echo=FALSE, fig=TRUE, height=4.5, width=7>>=
...
@
\caption{\label{fig:fatmodel_compare} Predicted 5\%, 50\%, and 95\% quantiles (in black) of
  the cyclic seasonal model along with the observed number of fatalities in 2000--2019
  (in light gray) and in 2020 (in red).}
\end{figure}

Figure~\ref{fig:fatmodel_compare} shows the predicted 5\%, 50\%, and 95\% quantiles (in black)
of the corresponding normal distributions along with the observed fatalities in 2000--2019
(in light gray) and in 2020 (in red, up to week 46), respectively. Thus, the quantiles reflect
the effects already conveyed by the predicted parameters in Figure~\ref{fig:fatmodel_effects}.
This shows that the fatalities in 2020 are above the median almost throughout all weeks
and above the 95\% quantile for a couple of weeks in spring and in the fall/winter, respectively.
While the mortality in the spring period is only moderately increased, it is much higher
than in previous years in fall/winter during the second COVID-19 wave in Austria.

In the following, we show how to draw Figure~\ref{fig:fatmodel_compare} using the
\pkg{bamlss} infrastructure. First, we set up a new data frame
and predict the distribution parameters for each week of the year.
<<eval=FALSE>>=
nd <- data.frame(week = 1:53)
par <- predict(b, newdata = nd, type = "parameter")
@
Based on these, the fitted quantiles can be computed using the quantile function
from the family of the model (see Section~\ref{sec:families} for details). The \fct{exp} transformation
maps the fitted values from the log-scale back to the original frequency scale.
<<eval=FALSE>>=
nd$fit <- sapply(c(0.05, 0.5, 0.95),
  FUN = function(p) { exp(family(b)$q(p, par)) })
@
Finally, the estimated quantiles and observed data can be visualized using \fct{matplot}
after reshaping the data to ``wide'' format with a separate column for every year.
...
\caption{\label{fig:fatmodel_resids} Histogram and Q-Q plot including 95\% credible
  intervals (dashed lines) of the randomized quantile residuals for the distributional
  regression model.}
\end{figure}

For judging how well the distributional model captures the observed data, the \fct{plot}
method includes some diagnostic graphics such as histograms or quantile-quantile (Q-Q) plots
of randomized quantile residuals \citep{bamlss:Dunn+Smyth:1996}, shown in Figure~\ref{fig:fatmodel_resids}.
<<eval=FALSE>>=
plot(b, which = c("hist-resid", "qq-resid"), c95 = TRUE)
@
By setting \code{c95 = TRUE}, the Q-Q plot includes 95\% credible intervals.
Both plots show that the log-transformation of the fatality numbers only partially captures
the right-skewed observations and that therefore the model fit is not ideal in the upper tail.
In an accompanying online vignette at \url{http://www.bamlss.org/articles/fatalities.html}
we show how to find a better probabilistic model by considering more flexible distributions
(beyond the normal case) and general Box-Cox transformations (instead of hard-coding the log-transformation).
While this improves the distributional model, the qualitative insights regarding the
excess mortality in Austria during the COVID-19 crisis remain unchanged.
...
\subsection{Model structure} \label{sec:mstruct}
...
(STAR,~\citealp{bamlss:Fahrmeir+Kneib+Lang:2004}). Similar to \citet{gamlss:book1},
\citet{bamlss:Umlauf+Klein+Zeileis:2018} relax this assumption and let $f_{jk}(\cdot)$ be an
unspecified composition of covariate data and regression coefficients. For example, functions
$f_{jk}(\cdot)$ could also represent nonlinear growth curves, a regression tree, a neural network or
lasso-penalized model terms as shown in Section~\ref{sec:flexm}.
...
Similarly, $\boldsymbol{\alpha}_{jk}$ is the set of all fixed prior specifications, i.e., for
GAM-type models $\boldsymbol{\alpha}_{jk}$ usually holds the so-called penalty matrices,
amongst others. In most
...
more detailed discussions on priors for $\boldsymbol{\tau}_{jk}$
and principled constructions for such priors).
...
\begin{itemize}
  \item Univariate responses of any type, e.g., counts with zero-inflation and or overdispersion as
    proposed in \citet{bamlss:Klein+Kneib+Lang:2015, bamlss:Herwartz+Klein+Strumann:2016, bamlss:Stasinopoulos+Rigby:2019},
    continuous responses with spikes, skewness, heavy tails or bounded support as in
    \citet{bamlss:Klein+Denuit+Kneib+Lang:2014,bamlss:Klein+Kneib+Lang+Sohn:2015,bamlss:Stasinopoulos+Rigby:2019}, as well as
 ...
 \item Multivariate responses with more complex dependence structures modeled through copulas
   \citep{bamlss:Klein+Kneib:2016b}.
...
separate updating equations using leapfrog or zigzag iteration
\citep{bamlss:Aitkin:1987, bamlss:Smyth:1996}, e.g., with updating equations
...
\citet{bamlss:Rigby+Stasinopoulos:2005} showed that using a basis function approach, i.e.,
each function $f_{jk}( \cdot )$ can be represented by a linear combination of a design matrix and
regression coefficients, the updating functions $U_{jk}( \cdot )$ for posterior mode (frequentist penalized likelihood) estimation for
...
well-known IWLS updating scheme for generalized linear models (GLM,~\citealp{bamlss:Nelder+Wedderburn:1972}). In the same way, approximate full conditionals
$\pi(\boldsymbol{\beta}_{jk} | \cdot)$ for MCMC are constructed with this
updating step \citep{bamlss:Gamerman:1997, bamlss:Fahrmeir+Kneib+Lang:2004, bamlss:Brezger+Lang:2006, bamlss:Klein+Kneib:2016b}.
...
$\mathbf{z}_k - \boldsymbol{\eta}_{k, -j}^{(t+1)}$ can be used (for detailed derivations see also
...
\item \emph{Information criteria} can be used to compare different model specifications.  
  ...
  complexity is based on the so-called equivalent degrees of freedom (EDF), i.e., for each
  model term the trace of the smoother matrix is computed (see, e.g.,
  \citealp{bamlss:Hastie+Tibshirani:1990}) and the total degrees of freedom
  are approximated by the sum over all distributional parameters and model terms.
...
\item Support for modular and exchangeable updating functions or complete model fitting engines
  ...
  posterior mode estimates from the \fct{optimizer} as starting values. An additional step can be used
...
\multirow{4}{*}{Estimation} & \multirow{2}{*}{Optimizer} & \code{opt\_bfit()}, \code{opt\_bbfit()},
 ...
 & \multirow{2}{*}{Sampler} & \code{sam\_GMCMC()}, \code{sam\_BayesX()}, \code{sam\_JAGS()} \\
...
<<eval=TRUE, echo=TRUE>>=
engines(NO, gaussian_bamlss, cox_bamlss)
@
...
These can only be used with the \fct{gaussian\_bamlss} family. In addition, neither \fct{NO} or
\fct{gaussian\_bamlss} has its own special optimizer or sampler implemented, such as the \fct{cox\_bamlss} family.
...
\code{sampler} is the \fct{sam\_GMCMC} sampling function, which is a quite generic implementation.
...
implemented in \proglang{C} and is additionally optimized for large design and penalty matrices
such that sampling using very large data sets is possible
(see \citealp{bamlss:Lang+Umlauf+Wechselberger+Harttgen+Kneib:2014} for details on algorithms,
e.g., using \fct{sam\_JAGS} is only suitable for moderate sized data and low
complexity model terms). For more details on \fct{sam\_GMCMC} please see the \pkg{bamlss}
manual.
...
using the \pkg{Formula} package infrastructure \citep{bamlss:Zeileis+Croissant:2010} in combination
...
\pkg{gamlss.dist} \citep{bamlss:gamlss.dist} package are supported, i.e., there is a transformer 
function that reads all necessary components and then transfers them into a family object for
\pkg{bamlss}.
...
\code{d} & A function returning the density with arguments \code{d(y, par, log = FALSE)}
  (see below). \\
\code{p} & The cumulative distribution function \code{p(y, par, ...)}. \\
...
For distributions for which the expectation of the second derivative is intractable or does not
exist, the user can rely on two options: the first option is to simply take
...
$\mathbf{W}_{kk}$. The second option is to find a good approximation for the expectation (see,
e.g., \citealp{bamlss:Klein+Kneib+Lang:2015}, for the case of the overdispersion parameter
...
\item a customized \fct{predict} function which will be called by \fct{predict.bamlss}, e.g., as
  implemented in the family \fct{cox\_bamlss},
\item similarly, a customized \fct{residuals} function that should be used by \fct{residuals.bamlss},
\end{itemize}
which can help to speed up optimization, or be convenient for
predictions and simulations.

When all formulas for a family are worked out, it usually takes about an hour to create a new
family object. Of course, this also depends on the complexity of the density function. With some
families it can be meaningful for speed reasons to port the functions for example to \proglang{C}.
In our experience, programming then takes only slightly longer, about 2 to 3 hours.
...
problem, only. As mentioned at the beginning of Section~\ref{sec:package}, there is naming
convention, optimizer functions start with prefix \code{opt\_*} and sampler functions with
\code{sam\_*}. The naming convention is not mandatory, but it gives the user a better overview of
the many functions of the package.
...
for the \fct{bamlss} wrapper function as long as a vector or matrix of parameters is
...
The reanalysis data come from ERA5, the fifth generation of the ECMWF (European
Centre for Medium-Range Weather Forecasts) atmosphheric reanalyses of the
global climate \citep{era5a, era5b}. ERA5 provides globally complete and consistent
...
\citep{FlashAustria}. The data are loaded with
%
<<echo=FALSE>>=
data("FlashAustria", package = "FlashAustria")
data("FlashAustriaModel", package = "FlashAustria")
FlashAustriaModel <- b <- flash_model_ztSICHEL
@
%
<<>>=
data("FlashAustria", package = "FlashAustria")
head(FlashAustriaTrain)[, 1:6]
nrow(FlashAustriaTrain)
@
%
The motivation for this application is as follows: lightning counts are not
modeled within the atmospheric reanalyses, as their spatial resolution is too
coarse for resolving convective events that lead to lightning discharges.
Homogeneous lightning observations are only available for the period in the
order of a decade, here 2010--2018. Thus, based on a probabilistic statistical
model, lightning counts for the time before 2010 could be fitted, thus enabling
the analysis of lightning events in the past for which no observations are available.
On the one hand this will increase our knowledge about physical processes leading
to such events, and on the other it will enable quantification how
these extreme short-term events are affected by changing climate \citep{westra2014}.

Table~\ref{tab:desc} lists the covariates considered which are based on a
small subset of ERA5 quantities \citep{era5a, era5b} and include variables that
are known to be good predictors for convective events
\citep[e.g.,][]{bamlss:Simon+Fabsic+Mayr+Umlauf+Zeileis:2018}.
...
\hline
\end{tabular}
\caption{\label{tab:desc} Quantities taken directly or derived from ERA5.}
\end{table}
...
truncated count data distribution which is highly
overdispersive \citep{cameron2013count}.
\citet{bamlss:Simon+Mayr+Umlauf+Zeileis:2019} employed a
zero-truncated negative binomial distribution, which
...
The spatial and temporal scale of aggregation of the lightning discharges here differs
from the one in \citet{bamlss:Simon+Mayr+Umlauf+Zeileis:2019}. Therefore, it is worth comparing
the zero-truncated negative binomial against other distributions that could capture the
truncation of the count data and its overdispersion. Hence, we also consider the
zero-truncated Sichel distribution which can also capture skewed count data.

The zero-truncated negative binomial distribution is implemented as
\fct{ztnbinom\_bamlss} within \pkg{bamlss} while the Sichel is available as
\fct{SICHEL} within \pkg{gamlss.dist}. Using the \pkg{gamlss.tr} package the latter
is truncated at zero so that it can be readily plugged into the \code{family}
argument of \fct{bamlss}.
%
<<>>=
library("gamlss.dist")
library("gamlss.tr")
...
@
%
In the following we illustrate how to set up and model the lightning counts with one
of the two distributions. To specify smooth terms for all distributional parameters
-- for \fct{ztnbinom\_bamlss} parameters $\mu$ and $\theta$ and for \fct{ztSICHEL}
parameters $\mu$, $\sigma$ and $\nu$ -- we set up a \code{list} of three formulas.
Smooth P-splines \citep{bamlss:Eilers+Marx:1996}, known for their good sampling properties,
are employed for all predictors in the formula for $\mu$. For the (over)dispersion
model large-scale precipitation is used in the second formula (without a parameter
name on the left-hand side in order to be applicable to both distributional models).
Finally, for the Sichel distribution a constant shape parameter is added in the third
formula (which is ignored when using the formula list with the zero-truncated negative
...
statistical models in \proglang{R}: a formula \code{f}, families
\fct{ztnbinom\_bamss}, \fct{ztSICHEL}, and a data set
\code{FlashAustriaTrain}. Within the
\fct{bamlss} call we also provide arguments which are passed forward to the
...
\citep{bamlss:Simon+Mayr+Umlauf+Zeileis:2019}. In the following, we illustrate the estimation of the 
models with the \fct{ztSICHEL} family.
We set the number of iterations to \code{maxit = 1000}. For the sampling we allow $1000$
iterations as burn-in phase, and apply a thinning of the resulting chain of $3$. Running
\code{n.iter = 2000} iterations on 3 cores in parallel leads to $1000$ MCMC samples in the
end (note that parallel chains are started using function \fct{mclapply} of the
base \proglang{R} \pkg{parallel} package by setting argument \code{cores}, see the manual of
sampler function \fct{sam\_MCMC}).
%
\begin{Schunk}
\begin{Sinput}
R> set.seed(123)
...
\end{Schunk}
%
The model was fitted on three Intel Xeon CPU E5-4660 v4 cores with 2.20GHz
on which the boosting took about 28.99~minutes and the average sampling time about 1.9~hours.
This is relatively slow as the \fct{ztSICHEL} uses a rather generic high-level \proglang{R}
implementation. To fit the model \code{flash\_model\_ztnbinom} we rather used
\code{family = ztnbinom\_bamlss()} than \code{family = ztSICHEL}, all other specifications for
...
\end{Schunk}
%
Computation times for the more efficient \fct{ztnbinom\_bamlss}
family are much lower with about 7 and 9~minutes for boosting and sampling, respectively.
By setting the argument \code{binning = TRUE} only unique observations are being used 
internally to create the design matrices. This can have two advantages, (a) for large data sets one 
does not run into memory problems so easily, (b) the estimation algorithms are sometimes much 
faster, because fewer floating point operations need to be performed. In addition, when setting
\code{light = TRUE}, no design or penalty matrices, model frames, results, etc.,
are stored. Hence, memory requirements when storing the model objects are much lower
so that the fitted model objects can be provided within the \pkg{FlashAustria} package.
<<eval=FALSE>>=
data("FlashAustriaModel", package = "FlashAustria")
@
The corresponding \proglang{R} code is provided in the supplemental materials.


\subsection{Model diagnostics}

To select one of the two models, we examine their calibration using a worm plot
\citep{bamlss:Buuren+Miranda:2001}. The worm plot is implemented within \pkg{bamlss}
for objects of class \code{bamlss.residuals} and can be selected via the \code{which}
argument of the plot method for these objects:
%
<<eval=FALSE>>=
set.seed(321)
...
\begin{figure}[t!]
\centering
<<fig=TRUE, height=4.5, width=6, echo=FALSE, pdf=FALSE, png=TRUE>>=
set.seed(321)
...
@
\caption{Worm plots for the two distributional models.}
\label{fig:appworm}
\end{figure}
%
The worm plots (Figure~\ref{fig:appworm}) reveal that both \code{ztSICHEL} and \code{ztnbinom} 
somewhat underestimate the mass of the upper tail. However, for \code{ztSICHEL} the effect
is less pronounced and overall calibration is much better than for \code{ztnbinom}.
Hence, we focus on the \fct{ztSICHEL} model but remark that most qualitative insights
are very similar for \code{ztnbinom}.

As a next diagnostic we check the log-likelihood contributions of the
individual terms during the boosting optimization (Figure~\ref{fig:appboost}).
%
<<eval=FALSE>>=
pathplot(flash_model_ztSICHEL, which = "loglik.contrib")
@
%
\setkeys{Gin}{width=.7\textwidth}
\begin{figure}[p!]
...
pathplot(flash_model_ztSICHEL, which = "loglik.contrib", intercept = TRUE, spar = FALSE)
## missing <- c("s(hcc).nu", "(Intercept).sigma")
## at_missing <- flash_model_ztSICHEL$model.stats$optimizer$boost_summary$loglik[1000, missing]
## axis(4, at = at_missing + c(2.8), labels = missing, las = 1, tick = FALSE)
@
...
\Sexpr{round(b$model.stats$optimizer$boost_summary$summary$mu["s(q_prof_PC1)",2])} %
followed by \code{s(sqrt_cape).mu} with %
\Sexpr{round(b$model.stats$optimizer$boost_summary$summary$mu["s(sqrt_cape)",2])}
and \code{s(d2m).mu} with
\Sexpr{round(b$model.stats$optimizer$boost_summary$summary$mu["s(d2m)",2])}.
%% These three terms as well as all other terms with a strong contribution
%% act on the distributional parameter \code{mu}.
Overall contributions to the log-likelihood at the end of the boosting procedure
are very small signalling that the algorithm approach a state that is suitable
for initializing the MCMC sampling.
...
@
\caption{MCMC trace (left panels) and autocorrelation (right panels) for two
parameters from the term \code{s(q\_prof\_PC1)} of the model \code{mu}.}
\label{fig:apptrace}
\end{figure}

The MCMC chains can be assessed by visualizations of their traces and autocorrelation functions (ACFs),
exemplified in Figure~\ref{fig:apptrace} for the term \code{s(q\_prof\_PC1)} (for parameter
$\mu$ of the Sichel distribution).
%
<<fig=FALSE, eval=FALSE>>=
plot(flash_model_ztSICHEL, model = "mu", term  = "s(q_prof_PC1)",
  which = "samples")
@
%
The traces reveal samples around stables means, confirming that the 1000 boosting
iterations and the 1000 burn-in samples were sufficient. The ACFs
reveal quite some autocorrelation after the thinning, suggesting that sampling efforts
should be increased further in a final model run.


\subsection{Predictions and visualizations}

%% As these diagnostics suggest that a reasonable initial state for the sampling
%% has been found,
%% and the samples are independent draws from the posterior
%% one can go further and investigate the estimated effects.

As the boosting summary (Figure~\ref{fig:appboost}) reveals that the terms
\code{s(q_prof_PC1)}, \code{s(sqrt_cape)} and \code{s(d2m)}
have the largest contribution for improving the fit, the corresponding
effects are shown in Figure~\ref{fig:appeffect} to illustrate how the
atmospheric quantities of the reanalyses are related to lightning events.
The effects are presented on the scale of the additive predictor of the
distributional parameter $\mu$, i.e., the log scale. A higher
$\log(\mu)$ would result in a higher expectation of the count data distribution.
%
<<fig=FALSE, eval=FALSE>>=
plot(flash_model_ztSICHEL, model = "mu",
...
@
\caption{Effect of the terms \code{s(q\_prof\_PC1)}, \code{s(sqrt\_cape)},
  and term \code{s(d2m)} from model \code{mu}. Credible intervals derived from MCMC
  samples.}
\label{fig:appeffect}
\end{figure}
%
\code{s(q_prof_PC1)} shows a clear decrease.
As \code{q_prof_PC1} is the leading principal component of the vertical profile
of specific humidity, one has to consider the corresponding spatial mode (not
shown) for interpretation: positive values of \code{q_prof_PC1} are linked to
more moisture in the lower atmosphere (below 850~hPa) and less moisture in the
mid atmosphere (between 850~hPa and 600~hPa). Thus, smaller values of the
principal component mean that more moisture is available in the mid atmosphere,
a source of \emph{latent energy}, energy that becomes free when water
transfers from the gas to the liquid phase. This energy supports the occurrence
of deep convection and thus of heavy lightning events.
%
\code{s(sqrt_cape)} reveals an increasing shape.
This means a higher \textit{convective available potential energy} (CAPE)
increases $\mu$, which increases the expectation of the distribution and
thus is associated with higher probabilities for events with high counts.
Physically the shape of the effect is meaningful as more convective available
potential energy has the potential to lead to heavier lightning events.
The similar is true for the increasing effect of \code{s(d2m)}.
%
\begin{figure}
...
September 15 2001 at 12~UTC and at 18~UTC and on September 16 2001 at
6~UTC and 12~UTC, i.e., the probability of having observed $10$ or more counts within one
grid box.}
\end{figure}
Finally, the model is leveraged to predict a case
...
observing 10 or more flashes within a grid box conditioned on thunderstorm activity,
by applying the cumulative distribution function \code{...$p()} of the family
which can be extracted from the fitted model using \code{family()}.
...
scores and Hessian. We apply the cdf to compute the probability of
observing more than 10 flashes in a box and hour given a lightning event.
The function \code{...$p()} takes the
quantile as first argument, and the \code{list} with the parameters,
as returned by \code{predict()}, as a second argument.
%
<<eval=FALSE>>=
library("sf")
fit <- predict(flash_model_ztSICHEL,
...
@
%
The case gives an example for a reconstructed lightning
event: On 2001-09-15 (top row of Figure~\ref{fig:appcase})
it reveals moderate probabilities for observing 10 or more
lightning counts within a grid cell and hour. Over night the situation is still unstable,
around 6~UTC on 2001-09-16 there is still quite high activity on the northern parts of the Alps.
(bottom left panel of Figure~\ref{fig:appcase}). Finally, strong lightning events were
reconstructed for the afternoon of the second day (2001-09-16) in particular on the Southern
side of the main Alpine ridge (bottom right panel of Figure~\ref{fig:appcase}).

\section{Conclusion}

The \proglang{R} package \pkg{bamlss} is a very comprehensive software to estimate Bayesian 
distributional regression models. The package is primarily based on the typical \proglang{R}
``look \& feel'', which makes it easy to get started with the package. Similar to other 
implementations, \pkg{bamlss} is a modular ``Lego toolbox'', however, the package stands out from
others in that it makes complete sampling and or optimizer functions exchangeable,
so that users who are interested in extensions can easily set up new models, where all
elaborate data processing infrastructure and extractor functions are completely provided by
the package.
Several examples illustrate the functionality of the package. For the future it is planned to 
provide algorithms for Gigadata, a topic that the package so far treats only very superficially.
In addition, it is planned to expand the family infrastructure to support families from other
implementations more easily.

\section*{Acknowledgments}

We thank two anonymous reviewers for critically reading the manuscript and suggesting
substantial improvements. Nadja Klein acknowledges
support through the Emmy Noether grant KL 3037/1-1 of the German research foundation (DFG). 
Thorsten Simon acknowledges the funding by the Austrian Science Fund (FWF, grant no.~P\,31836)
...
model terms that cannot be represented by the \pkg{mgcv} smooth term constructor infrastructure.
...
@
The estimated effects are shown in Figure~\ref{fig:GAMart} and can be plotted with:
<<eval=FALSE>>=
par(mfrow = c(1, 3))
...
plot2d(p.x3 ~ x3, data = nd, fill.select = c(0, 1, 0, 1), lty = c(2, 1, 2))
@
\caption{\label{fig:GAMart} Estimated effects for covariates \code{x1},
  \code{x2} and \code{x3} in the simulated data example.}
\end{figure}
