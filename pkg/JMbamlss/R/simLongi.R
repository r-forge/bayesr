...
#' New Simulation Function For Censored Longitudinal Data Based On FPCs
#' 
#' Adapt the structure given by simJM function in bamlss. Incorporate only 
#' non-informative censoring or no censoring at all.
#' 
...
#'   "ignoreDeg", "eValType" of function simMultiFunData and "eValScale" for
#'   scaling the eigenvalues.
simCensLongi <- function(nsub = 300, times = seq(0, 120, 1), probmiss = 0.75,
  ...
  rSurvtime <- function (censoring, x, r, tmin = 0, tmax, maxfac, ...)
  {
    nsub <- nrow(x)
    time <- rep(tmax, nsub)
    time_event <- censoring(time, tmax, maxfac)
    data_short <- data.frame(survtime = time_event[, 1], x, r)
  ...
  # Difference to simMultiJM
  data_short <- rSurvtime(censoring, x, r, tmin = times[1], tmax = tmax, 
                          maxfac = maxfac)
  data_short$id <- factor(seq_len(nsub)) 
  
  ## Create the full simulated data
  data_base <- cbind(data_short[id,], obstime = rep(times, nsub))
  ...
  # censoring                   
  data_cens <- data_long[data_long$obstime <= data_long$survtime,]
  
  # inducing longitudinal missings
  data_cens <- miss_fct(data_cens, probmiss, max_obs)
  data_uncens <- miss_fct(data_full, probmiss, max_obs)
  ...
  d <- list(data_uncens = data_uncens, data_cens = data_cens, 
            data_full = data_full, data_short = data_short,
