  ...
  sparse = TRUE, nonlinear = FALSE, edf_alt = FALSE, start_mu = NULL, k_mu = 6, ...)
{
  ...
  dalpha <- bamlss:::has_pterms(x$dalpha$terms) | (length(x$dalpha$smooth.construct) > 0)
  ...
  for(j in c("lambda", "gamma", "alpha", if(dalpha) "dalpha" else NULL)) {
    if(nonlinear & (j =="alpha")){
      x[[j]]<- bamlss:::design.construct(terms, data = data[take_last, , drop = FALSE], knots = list(mu = k),
                 model.matrix = TRUE, smooth.construct = TRUE, model = j,
                 scale.x = FALSE, absorb.cons = TRUE, C = C)[[j]]
    } else {
      x[[j]]<- bamlss:::design.construct(terms, data = data[take, , drop = FALSE], knots = knots,
                 model.matrix = TRUE, smooth.construct = TRUE, model = j,
                 scale.x = FALSE)[[j]]
    }
  }
  ...
  if(dalpha) {
    ...
    x$dmu <- bamlss:::design.construct(terms, data = data, knots = knots,
  ...
  for (j in c("lambda", if(nonlinear) "alpha" else NULL)){
    ...
    if("(Intercept)" %in% cn)
      ...
      if(ncol(x[[j]]$smooth.construct$model.matrix$X) < 1) {
        ...
        x[[j]]$terms <- bamlss:::drop.terms.bamlss(x[[j]]$terms, pterms = FALSE, keep.intercept = FALSE)
    } else {
        ...
        x[[j]]$terms <- bamlss:::drop.terms.bamlss(x[[j]]$terms, keep.intercept = FALSE) # instead of attr(x[[j]]$terms, "intercept") <- 0 
  ...
  for(i in seq_along(ntd)) {
    if(bamlss:::has_pterms(x[[ntd[i]]]$terms)) {
      if(ntd[i] %in% c("lambda", if(nonlinear) "alpha")) {
        x[[ntd[i]]]$smooth.construct$model.matrix <- param_time_transform2(x[[ntd[i]]]$smooth.construct$model.matrix,
                                                                           bamlss:::drop.terms.bamlss(x[[ntd[i]]]$terms, sterms = FALSE, keep.response = FALSE), data, grid, yname,
                                                                           timevar_mu, take, timevar2 = timevar_mu, idvar = idvar, delete.intercept = TRUE)
      } else {
        x[[ntd[i]]]$smooth.construct$model.matrix <- bamlss:::param_time_transform(x[[ntd[i]]]$smooth.construct$model.matrix,
                                                                          bamlss:::drop.terms.bamlss(x[[ntd[i]]]$terms, sterms = FALSE, keep.response = FALSE), data, grid, yname, 
  ...
  for(j in names(x)) {
    for(sj in names(x[[j]]$smooth.construct)) {
      priors <- bamlss:::make.prior(x[[j]]$smooth.construct[[sj]])
...
sparse_Matrix_setup <- function(x, sparse = TRUE, force = FALSE, take, nonlinear)
{
  if(sparse & ((ncol(x$sparse.setup$crossprod) < (ncol(x$X) * 0.5)) | force)) {
      ...
      x$XT <- Matrix:::Matrix(x$XT, sparse = TRUE)
...
opt_JM <- jm_mode <- function(x, y, start = NULL, weights = NULL, offset = NULL,
  ...
  verbose = TRUE, digits = 4, fix.lambda = FALSE, fix.gamma = FALSE, fix.alpha = FALSE,
  fix.mu = FALSE, fix.sigma = FALSE, fix.dalpha = FALSE, ...)
{
  ## Hard coded.
  #fix.lambda <- FALSE
  #fix.gamma <- FALSE
  ...
  while((eps0 > eps[1]) & (iter < maxit[1])) {
    ...
    edf <- bamlss:::get.edf(x, type = 2)
    IC <- bamlss:::get.ic2(logLik, edf, length(eta$mu), criterion)
    ...
    if(verbose) {
      cat(if(ia) "\r" else "\n")
      vtxt <- paste(
        criterion, " ", bamlss:::fmt(IC, width = 8, digits = digits),
        " logLik ", bamlss:::fmt(logLik, width = 8, digits = digits),
        " edf ", bamlss:::fmt(edf, width = 6, digits = digits),
        " eps ", paste(bamlss:::fmt(eps0, width = 6, digits = digits + 2), "|S",
                       bamlss:::fmt(eps0_surv, width = 6, digits = digits + 2), "|L",
                       bamlss:::fmt(eps0_long, width = 6, digits = digits + 2), "|A",
                       bamlss:::fmt(eps0_alpha, width = 6, digits = digits + 2), sep = ""),
  ...
  return(list("fitted.values" = eta, "parameters" = bamlss:::get.all.par(x),
              "logLik" = logLik, "logPost" = logPost, "hessian" = bamlss:::get.hessian(x),
                            ...
                            status, update.nu, criterion, get_LogPost, nobs, do.optim2, edf, ...)
{
  ...
  if((!(!x$state$do.optim | x$fixed | x$fxsp)) & do.optim2) {
    ...
    objfun1 <- function(tau2) {
      ...
      Sigma <- bamlss:::matrix_inv(xhess, index = x$sparse.setup)
      Hs <- Sigma %*% xgrad
      g <- par[x$pid$b]
      if(update.nu) {
      ...
      fit <- x$fit.fun(x$X, g2)
      eta$gamma <- eta$gamma - fitted(x$state) + fit
      edf1 <- bamlss:::sum_diag(xhess0 %*% Sigma)
      edf <- edf0 + edf1
    ...
    tau2 <- bamlss:::tau2.optim(objfun1, start = get.state(x, "tau2"))
  ...
  Sigma <- bamlss:::matrix_inv(xhess, index = NULL)
  Hs <- Sigma %*% xgrad
  ...
  x$state$edf <- bamlss:::sum_diag(xhess0 %*% Sigma)
                             ...
                             status, update.nu, width, criterion, get_LogPost, nobs, do.optim2, edf, nonlinear, ...)
{
  ...
  # int <- bamlss:::survint(X, eeta, width, exp(eta$gamma), index = x$sparse.setup$matrix) # throws error with time-varying covariates
  int <- bamlss:::survint(X, eeta, width, exp(eta$gamma))
  ...
  if((!(!x$state$do.optim | x$fixed | x$fxsp)) & do.optim2) {
    ...
    objfun1 <- function(tau2) {
      ...
      edf1 <- bamlss:::sum_diag(int$hess %*% Sigma)
      edf <- edf0 + edf1
      logLik <- get_LogPost(eta_timegrid, eta, 0)
      ic <- bamlss:::get.ic2(logLik, edf, length(eta$mu), criterion)
      if(!is.null(env$ic_val)) {
  ...
  x$state$edf <- bamlss:::sum_diag(int$hess %*% Sigma)
  x$state$hessian <- xhess
                         ...
                         dx = NULL, xsmalpha = NULL, knots = NULL, ...)
{
  ...
  if((!(!x$state$do.optim | x$fixed | x$fxsp)) & do.optim2) {
    ...
    objfun1 <- function(tau2) {
      ...
      if(!is.null(dx))
        eta_timegrid_dmu <- eta_timegrid_dmu - dx$state$fitted_timegrid + dx$fit.fun_timegrid(g2)
      eta_timegrid <- eta_timegrid_lambda + eta_timegrid_alpha * eta_timegrid_mu + eta_timegrid_dalpha * eta_timegrid_dmu
      edf1 <- bamlss:::sum_diag((-1 * xhess0) %*% Sigma)
      edf <- edf0 + edf1
      logLik <- get_LogPost(eta_timegrid, eta, 0)
      ic <- bamlss:::get.ic2(logLik, edf, length(eta$mu), criterion)
      if(!is.null(env$ic_val)) {
    ...
    assign("ic00_val", objfun1(get.state(x, "tau2")), envir = env)
    tau2 <- bamlss:::tau2.optim(objfun1, start = get.state(x, "tau2"), maxit = 1)
  ...
  x$state$edf <- bamlss:::sum_diag((-1 * xhess0) %*% Sigma)
                            ...
                            status, update.nu, width, criterion, get_LogPost, nobs, do.optim2, edf, ...)
{
  ...
  ## Compute gradient and hessian integrals.
  int <- bamlss:::survint(X, eeta * eta_timegrid_mu, width, exp(eta$gamma),
  ...
  ## Compute the inverse of the hessian.
  Sigma <- bamlss:::matrix_inv(xhess, x$sparse.setup)
  Hs <- Sigma %*% xgrad
                             ...
                             status, update.nu, width, criterion, get_LogPost, nobs, do.optim2, edf, ...)
{
  ...
  ## Compute gradient and hessian integrals.
  int <- bamlss:::survint(X, eeta * eta_timegrid_dmu, width, exp(eta$gamma),
  ...
  prop_list = FALSE, ...)
{
  ...
  ## Compute additive predictors.
  eta <- bamlss:::get.eta(x, expand = FALSE) # potentially include alpha_mu
                              ...
                              width, sub, nu, status, id, verbose_sampler, ...)
{
  ...
  ## Compute gradient and hessian integrals.
  int <- bamlss:::survint(X, eeta, width, exp(eta$gamma), index = x$sparse.setup$matrix)
  xgrad <- drop(t(status) %*% x$XT - int$grad)
  xgrad <- xgrad + x$grad(score = NULL, x$state$parameters, full = FALSE)
  xhess <- int$hess + x$hess(score = NULL, x$state$parameters, full = FALSE)
  ...
  Sigma2 <- bamlss:::matrix_inv(xhess, index = NULL)
  mu2 <- drop(g + nu * Sigma2 %*% xgrad)
  qbeta <- dmvnorm(g0, mean = mu2, sigma = Sigma2, log = TRUE)
  
  ## Save edf.
  x$state$edf <- bamlss:::sum_diag(int$hess %*% Sigma2)
                                 ...
                                 verbose_sampler, ...)
{
  ...
  ## Save edf.
  x$state$edf <- bamlss:::sum_diag((-1 * int$hess) %*% Sigma2)
                             ...
                             verbose_sampler, ...)
{
  ...
  ## Save edf.
  x$state$edf <- bamlss:::sum_diag(xhess0 %*% Sigma2)
                             ...
                             verbose_sampler, ...)
{
  ...
  ## Save edf.
  x$state$edf <- bamlss:::sum_diag((-1 * xhess0) %*% Sigma2)
...
param_time_transform2 <- function (x, formula, data, grid, yname, timevar, take, 
  ...
  x$XT <- bamlss:::extract_XT(X, gdim[1], gdim[2])
  if (derivMat) {
                       ...
                       chunks = 1, verbose = FALSE,  ...)
{
  ...
  if(object$family$family != "jm")
    ...
    dalpha <- bamlss:::has_pterms(object$x$dalpha$terms) | (length(object$x$dalpha$smooth.construct) > 0)
  ...
  jm_probs <- function(data) {
    ...
    pred_lambda <- with(pred.setup, .predict.bamlss.surv.td("lambda",
                                                            ...
                                                            bamlss:::drop.terms.bamlss(object$x$lambda$terms, sterms = FALSE, keep.response = FALSE),
                                                            type = 2))
                               ...
                               update_data = NULL)
{
  ...
  if(derivMat) {
    dX <- PredictMat(x, dX)
    X <- -1 * (X - dX) / eps
    x$XT <- bamlss:::extract_XT(dX, gdim[1], gdim[2])
    x$X <- -1 * (x$X - PredictMat(x, ddata)) / eps
    gm <- bamlss:::sparse.matrix.index(X)
    if(!is.null(gm))
      x$sparse.setup$grid.matrix <- gm
  } else {
    x$sparse.setup$grid.matrix <- bamlss:::sparse.matrix.index(X)
  }
  ff <- bamlss:::make.fit.fun(x, type = 2)
                                  ...
                                  status, update.nu, width, criterion, get_LogPost, nobs, do.optim2, edf, edf_alt = FALSE, tp = FALSE, ...)
{
  ...
  ## Compute gradient and hessian integrals.
  int <- bamlss:::survint(X, eeta, width, exp(eta$gamma), index = NULL)
  ...
  if((!(!x$state$do.optim | x$fixed | x$fxsp)) & do.optim2) {
    ...
    objfun1 <- function(tau2) {
      ...
      S <- int$hess %*% Sigma
      if(edf_alt){
        edf1 <- bamlss:::sum_diag(2*S - S%*%S)
      } else {
        edf1 <- bamlss:::sum_diag(S)
      }
                               ...
                               dx = NULL, xsmalpha = NULL, knots = NULL, tp = FALSE, fac = FALSE, ...)
{
  ...
  ## Compute gradient and hessian integrals.
  int <- bamlss:::survint(X, eeta * eta_timegrid_alpha1, width, exp(eta$gamma),
