...
opt_MJM <- function(x, y, start = NULL, eps = 0.0001, maxit = 600, nu = 0.1, ...)
{
  ...
  eta0_surv <- eta0_surv[take_last,]
  eta0_alpha <- matrix(eta$alpha[take_last], nrow = nsubj, ncol = nmarker)
  ...
  while((eps0 > eps) & (iter < maxit)) {
    ...
    # Vektor.
    eta_T_long <- drop(
      t(rep(1, nmarker)) %x% diag(nsubj) %*% (eta$alpha[take_last_l] * eta_T_mu))
    eta_T <- eta$lambda[take_last] + eta$gamma[take_last] + eta_T_long
    
    sum_Lambda <- (survtime/2 * exp(eta$gamma[take_last])) %*%
    ...
    # Stopping criterion
    iter <- iter + 1
    eta1_surv <- do.call("cbind", eta[c("lambda", "gamma")])[take_last, ]
    eta1_alpha <- matrix(eta$alpha[take_last], nrow = nsubj, ncol = nmarker)
...
update_mjm_lambda <- function(x, y, nu, eta, eta_timegrid, survtime, ...)
{
  ...
  int_i <- survint_gq(pred = "lambda", pre_fac = exp(eta$gamma[attr(y, "take_last")]),
...
update_mjm_gamma <- function(x, y, nu, eta, eta_timegrid, survtime, ...) {
  ...
  int_i <- survint_gq(pred = "gamma", pre_fac = exp_eta_gamma, pre_vec = x$X[take_last, , drop = FALSE],
                      omega = exp(eta_timegrid),
                      weights = attr(y, "gq_weights"),
                      survtime = survtime)
  x_score <- drop(attr(y, "status") %*% x$X[take_last, , drop = FALSE]) - colSums(int_i$score_int)
  x_H <- matrix(colSums(int_i$hess_int), ncol = b_p)
...
update_mjm_alpha <- function(x, y, nu, eta, eta_timegrid, eta_timegrid_mu, 
  ...
  int_i <- survint_gq(pred = "long", pre_fac = exp(eta$gamma[take_last]),
                      omega = exp(eta_timegrid),
