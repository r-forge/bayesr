...
d_indeprirs$data$obstime <- d_indeprirs$data$obstime / 25
d_indeprirs$data$survtime <- d_indeprirs$data$survtime / 25
...
f_indeprirs_pcre <- list(
  ...
  gamma ~ 1 + x3,
  mu ~ -1 + marker + obstime:marker + x3:marker + obstime:marker:x3 +
    s(id, fpc.1, fpc.2, bs = "unc_pcre",
      xt = list("mfpc" = 
                  list(functions = extractObs(mfpca_indeprirs$functions, 1:2),
                       values = c(1, 1)))) +
    s(id, fpc.3, fpc.4, bs = "unc_pcre",
      xt = list("mfpc" = 
...
sink()
set.seed(1808)
sink("indeprirs_pcre_est401.txt")
