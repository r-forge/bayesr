...
# Final choice
mfpca_w <- JMbamlss:::preproc_MFPCA(
...
saveRDS(bamlss_wei, file = file.path(results_wd, "inst", "objects", 
                                     "pbc_analysis", "230927_pbc_fit_w.rds"))
...
# Prepare objects for model fit
p_long[, grepl("fpc", colnames(p_long))] <- NULL
p_long <- JMbamlss:::attach_wfpc(mfpca_u, p_long, n = length(vals))
...
set.seed(1209)
t_w <- system.time(
  bamlss_unw <- bamlss(f_est, family = JMbamlss:::mjm_bamlss, data = p_long,
                       timevar = "obstime", maxit = 1500,
                       n.iter = 12000L, burnin = 2000L, thin = 5)
)
saveRDS(bamlss_unw, file = file.path(results_wd, "inst", "objects", 
                                     "pbc_analysis", "230927_pbc_fit_u.rds"))
...
saveRDS(jointFit, file = file.path(results_wd, "inst", "objects", 
                                   "pbc_analysis", "230927_pbc_fit_jmb.rds"))
