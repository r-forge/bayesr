...
# Add noise FPCs
m1sp4 <- splinefun(x, c(0, 0.2, 0.3, 0.4, 0.2, 0, 0.2, 0.4, 0.6, 0.4, 0.2))
m2sp4 <- splinefun(x, c(0, 0.4, 0.2, 0.2, 0, -0.1, -0.2, -0.2, 0, 0, 0.2))
m1sp5 <- splinefun(x, c(0.2, 0.2, 0, -0.2, -0.2, 0, 0.2, 0.2, 0.4, 0.2, 0.2))
m2sp5 <- splinefun(x, c(0.2, 0.4, 0.6, 0.3, 0.2, 0, 0, 0.2, 0.2, 0, -0.2))
m$values <- c(m$values, 0, 0)
intw <- .intWeights(m$functions@.Data[[1]]@argvals[[1]])
X1 <- m$functions@.Data[[1]]@X
X2 <- m$functions@.Data[[2]]@X
X <- t(t(cbind(X1, X2)) * sqrt(intw))
X <- rbind(X, c(m1sp4(argvals), m2sp4(argvals)), 
           c(m1sp5(argvals), m2sp5(argvals)))

X[7, ] <- X[7, ] - drop(X[1, ] %*% X[7, ])*X[1, ] -
  drop(X[2, ] %*% X[7, ])*X[2, ] -
  drop(X[3, ] %*% X[7, ])*X[3, ] -
  drop(X[4, ] %*% X[7, ])*X[4, ] -
  drop(X[5, ] %*% X[7, ])*X[5, ] -
  drop(X[6, ] %*% X[7, ])*X[6, ]
X[7, ] <- X[7, ] / sqrt(drop(X[7, ] %*% X[7, ]))
X[8, ] <- X[8, ] - drop(X[1, ] %*% X[8, ])*X[1, ] -
  drop(X[2, ] %*% X[8, ])*X[2, ] -
  drop(X[3, ] %*% X[8, ])*X[3, ] -
  drop(X[4, ] %*% X[8, ])*X[4, ] -
  drop(X[5, ] %*% X[8, ])*X[5, ] -
  drop(X[6, ] %*% X[8, ])*X[6, ] -
  drop(X[7, ] %*% X[8, ])*X[7, ]
X[8, ] <- X[8, ] / sqrt(drop(X[8, ] %*% X[8, ]))

X <- t(t(X) / sqrt(intw))
m$functions <- multiFunData(funData(m$functions@.Data[[1]]@argvals, X[, 1:101]),
                            funData(m$functions@.Data[[2]]@argvals, X[, 102:202]))
...
parallel_bamlss_est <- function(i) {
  ...
  try_obj <- try({
    ...
    # Use all FPCs 
    nfpc <- nObs(m$functions)
    d_rirs_est <- d_rirs$data %>%
      select(!matches("fpc"))
    d_rirs_est <- JMbamlss:::attach_wfpc(m, data = d_rirs_est)
