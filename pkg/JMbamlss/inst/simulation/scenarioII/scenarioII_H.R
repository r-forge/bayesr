...
# Rotate the MFPC basis
X_mfpc <- t(cbind(m$functions@.Data[[1]]@X, m$functions@.Data[[2]]@X))
R_mat <- fda::varmx(X_mfpc)
X_rotate <- X_mfpc %*% R_mat
m$functions <- multiFunData(funData(m$functions@.Data[[1]]@argvals,
                                    t(X_rotate)[, 1:101]),
                            funData(m$functions@.Data[[2]]@argvals,
                                    t(X_rotate)[, 102:202]))
