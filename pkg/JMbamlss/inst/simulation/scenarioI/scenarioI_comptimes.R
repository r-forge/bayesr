...
# Different numbers of FPCs
nfpc_est1 <- length(mfpca_est$values)
nfpc_est99 <- min(which(
...
mfpca_est_list <- lapply(vals[seq_len(nfpc_est1)], 
...
d_rirs_est1 <- JMbamlss:::attach_wfpc(mfpca_est, d_rirs$data, n = nfpc_est1)
f_est <- list(
...
# Model fit
t_est1 <- system.time(
  b_est <- bamlss(f_est, family = JMbamlss:::mjm_bamlss, data = d_rirs_est1, 
...
# Prepare objects for model fit
d_rirs_est99 <- JMbamlss:::attach_wfpc(mfpca_est, d_rirs$data, n = nfpc_est99)
f_est <- list(
  Surv2(survtime, event, obs = y) ~ -1 + s(survtime, k = 20, bs = "ps"),
  gamma ~ 1 + x3,
  as.formula(paste0(
    "mu ~ -1 + marker + obstime:marker + x3:marker + obstime:x3:marker +",
    paste0(lapply(seq_len(nfpc_est99), function(x) {
      paste0("s(id, fpc.", x, ", bs = 'unc_pcre', xt = list('mfpc' = ",
             "mfpca_est_list[[", x, "]], 'scale' = FALSE))")
    }), collapse = " + "))),
  sigma ~ -1 + marker,
...
# Model fit
t_est99 <- system.time(
  b_est <- bamlss(f_est, family = JMbamlss:::mjm_bamlss, data = d_rirs_est99, 
                  timevar = "obstime", maxit = 1500, n.iter = 5500,
                  burnin = 500, thin = 5)
)
rm(b_est)
...
# Prepare objects for model fit
d_rirs_est975 <- JMbamlss:::attach_wfpc(mfpca_est, d_rirs$data, n = nfpc_est975)
f_est <- list(
  Surv2(survtime, event, obs = y) ~ -1 + s(survtime, k = 20, bs = "ps"),
  gamma ~ 1 + x3,
  as.formula(paste0(
...
t_est975 <- system.time(
  b_est <- bamlss(f_est, family = JMbamlss:::mjm_bamlss, data = d_rirs_est975, 
                  timevar = "obstime", maxit = 1500, n.iter = 5500,
                  burnin = 500, thin = 5)
)
rm(b_est)
...
# Prepare objects for model fit
d_rirs_est95 <- JMbamlss:::attach_wfpc(mfpca_est, d_rirs$data, n = nfpc_est95)
f_est <- list(
  Surv2(survtime, event, obs = y) ~ -1 + s(survtime, k = 20, bs = "ps"),
  gamma ~ 1 + x3,
  as.formula(paste0(
...
# Model fit
t_est95 <- system.time(
  b_est <- bamlss(f_est, family = JMbamlss:::mjm_bamlss, data = d_rirs_est95, 
                  timevar = "obstime", maxit = 1500, n.iter = 5500,
                  burnin = 500, thin = 5)
)
rm(b_est)
...
# Computation times
# (b_est975 contains 8 FPCs)
#
# > t_est1
# user   system  elapsed 
# 1821.747    7.305 1827.763 
# > t_est99
# user   system  elapsed 
# 1542.744   10.138 1552.460 
# > t_est975
# user   system  elapsed 
# 1243.349    0.710 1243.709 
# > t_est95
# user  system elapsed 
# 964.232   0.851 964.879 
# > t_jm
# user  system elapsed 
# 51.479   0.230  51.724 